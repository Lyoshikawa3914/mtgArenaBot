...
# allows program to read text from an image
import pytesseract
import cv2

#random library in case I need to use anything random
import random
...
def changeAvatar():
    ...
    #click the 'profile' tab on upper part of screen
    mousePos((288, 28))
    time.sleep(2)
    leftClick()

    #clicking 'Change Avatar' button on bottom right
    mousePos((1199, 748))
    time.sleep(2)
    leftClick()
...
def image_check():
    ...
    box = (54, 287, 1256, 471) #best working cords (5, 769, 200, 826)
    
    pytesseract.pytesseract.tesseract_cmd = r"C:\Program Files\Tesseract-OCR\tesseract.exe"

    im = ImageGrab.grab(box)
    #pixel = im.getpixel((1898, 33))
...
class bot():
    ...
    def login_screen(self):
        ...
        im = CordsToImage(54, 287, 1256, 471)
        for i in im:
            if i == "Unknown" or i == "Error":
                mousePos((976, 530)) #moves cursor to the retry button
                leftClick()
    ...
    def getdeck(self):
        ...
        #quest
            ...
            print("you are playing a non-color quest")
            no_color_quest = True
        ...
        #randomly select a deck if the quest is a non-color quest
        else:
    ...
    def gameplay(self):
        ...
        #original sum(reference) in range(302, 315) = symbol on bottom left
        while color == reference_color or sum(reference_color) in range(302, 315): 
            ...
            if sum(pixel) in range(300, 490):
                ...
                if count >= 23:
                    start = 2
                for i in range(start, len(hand)-start):
                #for i in range(start, HandDuration):
                    ...
                    if aura[2] >= 102 or sum(aura) in range(376, 674):
                        doubleClick()
                        time.sleep(.5)
                        mousePos((22, 598))
                        time.sleep(.7)
...
bot5 = bot(email, password)

accounts = [bot1, bot2, bot3, bot4, bot4, bot5]
#"""
...
for account in accounts:
    ...
    while time.time() < endtime:
        print("starting bot ", count)
        account.getdeck()
        
        account.gameplay()
        time.sleep(20)

        #fail-safe action in case a quest is completed
        mousePos((21, 474))
        leftClick()

        print("current time", time.time())
        print("when to end the session", endtime)
        
    print("ending bot " , count)
    time.sleep(7)
    account.logout()
    count += 1
    endtime = time.time() + 60*60
